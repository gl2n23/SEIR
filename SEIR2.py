...
class SEIRModel(Model):
    ...
    def get_next_day_routines(self):
        ...
        if self.schedule.steps % 48 == 0:
            ...
            print('exposed+infecting rate', expo_infe_number / agent_number)
            ...
            for agent in self.schedule.agents:
                ...
                if isinstance(agent, MovingAgent):
                    ...
                    if expo_infe_number/agent_number >= 0.1:  # 设置封控 当感染率大于0.1时开始封控
                        lockdown = True
                    elif expo_infe_number/agent_number == 0:  # 直到感染率为0
                        lockdown = False

                    message = message_select(expo_infe_number, agent_number, agent.infected, agent.time_infected, agent.recovered,
